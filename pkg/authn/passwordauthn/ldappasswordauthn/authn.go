...
func (a *AuthN) getLDAPConfig(ctx context.Context, identifier string, orgID valuer.UUID) (*authtypes.LdapConfig, error) {
	...
	for _, domain := range domains {
		if domain.Enabled() && domain.Kind() == authtypes.AuthNProviderLDAP {
			if ldapConfig, err := domain.Config().LdapConfig(); err == nil {
	...
	if !strings.Contains(identifier, "@") {
		...
		if defaultDomainName != "" {
			...
			if err == nil && domain.Enabled() && domain.Kind() == authtypes.AuthNProviderLDAP {
				if ldapConfig, err := domain.Config().LdapConfig(); err == nil {
					return &ldapConfig, nil
				}
			}
