...
func buildLogsQuery(panelType v3.PanelType, start, end, step int64, mq *v3.BuilderQuery, graphLimitQtype string) (string, error) {
	...
	queryTmpl =
		queryTmpl + selectLabels +
			" %s as value " +
			"from " + logstelemetryschema.DBName() + ".distributed_logs " +
			"where " + timeFilter + "%s" +
			"%s%s" +
			"%s"
	...
	case v3.AggregateOperatorNoOp:
		queryTmpl := constants.LogsSQLSelect + "from " + logstelemetryschema.DBName() + ".distributed_logs where %s%s order by %s"
...
func buildLogsLiveTailQuery(mq *v3.BuilderQuery) (string, error) {
	...
	switch mq.AggregateOperator {
	case v3.AggregateOperatorNoOp:
		query := constants.LogsSQLSelect + "from " + logstelemetryschema.DBName() + ".distributed_logs where "
		if len(filterSubQuery) > 0 {
